...
const KeywordCard: React.FC<Props> = ({ keyword, isFavorite, onToggleFavorite, onClick, isSelectedForFusion, isFusionMode }) => {
    ...
    return (
        ...
        >
            ...
            <div className={`bg-gradient-to-br ${colorClass} p-4 rounded-xl mb-4`}>
                <h3 className="text-xl font-bold mb-2">{keyword.term}</h3>
            ...
            <div className="flex flex-wrap gap-2 mt-auto">
                 {keyword.crossDomains.slice(0, 3).map(domain => (
                    <span key={domain} className="flex items-center gap-1 text-xs px-2 py-1 bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 rounded-full">
                        <Network size={10} />
                        {domain}
                    </span>
                 ))}
                 {keyword.crossDomains.length > 3 && <span className="text-xs text-gray-400 self-center">...</span>}
            </div>
        </motion.div>
    );
};
