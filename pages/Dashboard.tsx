...
import { Search, Filter, Shuffle, Zap, Heart, Download, BookOpen, Settings } from 'lucide-react';
...
const Dashboard: React.FC = () => {
    ...
    const [modalData, setModalData] = useState<{ data: any, isFusion: boolean } | null>(null);
    ...
    const handleRandom = () => {
        const pool = filteredKeywords.length > 0 ? filteredKeywords : keywords;
    ...
    const handleFusionGenerate = () => {
        const selectedKeywords = keywords.filter(kw => fusionSelection.includes(kw.term));
    ...
    const handleExport = () => {
        const data = {
            favorites: keywords.filter(kw => favorites.includes(kw.term)),
                ...
                <div className="container mx-auto px-4 py-4">
                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <Link
                                to="/admin"
                                className="flex items-center gap-2 px-3 py-2.5 rounded-xl font-medium bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-700 transition-all"
                                title="管理后台"
                            >
                                <Settings size={18} />
                            </Link>
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium transition-all ${fusionMode
